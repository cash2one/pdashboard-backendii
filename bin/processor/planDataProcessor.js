...
module.exports = new Processor({
    ...
    finish: function (evt) {
        ...
        if (this.logs.length) {
            pendingJobs.push(
                this.updateLogs(db, 'performance_plan_basic', cover2document(this.logs))
            );
        }
        if (this.hourlyLogs.length) {
            pendingJobs.push(
                this.updateLogs(db, 'performance_plan_basic_hourly', cover2document(this.hourlyLogs))
            );
        }
