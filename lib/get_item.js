 ...
 *  ---   获取 线下 20121030的涅槃日志后端处理统计任务结果（小时级）
 * node get_item.js fclog_back_view_out_t -s 20121030 -h 3600 -o -b 01:00
 *  ---   获取 线下 20121030的1点涅槃日志后端处理统计任务结果（小时级）
 *
 */
...
function getItem(argv) {
    ...
    if (!BEGIN_DATE) {
        BEGIN_DATE = date.add('d', -1).strftime('%Y%m%d');
    }
    var END_DATE = argv.f || '';
    if (!END_DATE) {
        END_DATE = BEGIN_DATE;
...
function getItemFromHadoop(logName, callDate, callTime, saveFolder, isOffline, needMd5) {
    var SAVE_FOLDER = '';
    if (saveFolder === '.') {
        SAVE_FOLDER = 'logs/' + saveFolder + '/' + logName + '/' + callDate + '/' + callTime;
    }
    else {
    ...
    var COMMAND = 'hadoop fs -get ' + DOWNLOAD_DIR + ' ' + SAVE_FOLDER;
